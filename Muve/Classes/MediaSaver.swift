...
class MediaSaver: NSObject {
    ...
    static let sharedInstance = MediaSaver()
    ...
    func createAlbum() {
        PHPhotoLibrary.sharedPhotoLibrary().performChanges({
            PHAssetCollectionChangeRequest.creationRequestForAssetCollectionWithTitle(MediaSaver.albumName)   // create an asset collection with the album name
        }) { success, error in
            if success {
    ...
    func fetchAssetCollectionForAlbum() -> PHAssetCollection! {
        ...
        fetchOptions.predicate = NSPredicate(format: "title = %@", MediaSaver.albumName)
